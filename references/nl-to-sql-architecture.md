...
**这种做法无异于烧美元驱动蒸汽机**：投入极高的算力成本（Token 燃烧），却换来极低的吞吐效率（热值不够），是一种疯狂的浪费。

### AI 应用核心原则

**造电机与方向盘**
- **错误（烧美元）**：把数据直接扔给 AI 处理。AI 是极其昂贵的“设计师”，不是通用的“苦力”。让它做简单的数据过滤和聚合，无异于让总工程师去搬砖，不仅贵，而且慢。
- **正确（造电机/握方向盘）**：
  - **AI 是造电动机的人 / 方向盘**：对于复杂的意图理解（NL -> SQL），AI 是高效的“转换器”和“指挥官”。它的价值在于**设计指令**（造电机）和**控制方向**（路由），而不是**出力**。
  - **DuckDB 是电动机**：对于确定的计算（聚合、过滤、排序），使用廉价且高效的专用引擎。它是被造出来的机器，负责提供巨大的吞吐“马力”，且成本极低。

本架构的所有决策都遵循这一原则：**AI 负责“造指令”（生成 SQL）和“指引方向”，DuckDB 负责“出力”（执行计算）。**
...
数据同步需要调用 API，必须解决**"谁有权获取数据"**的问题。

### 被排除：用户级同步（Sync 复用查询用户身份）

即每次查询时，Sync 使用该用户的 Token 去拉取增量数据。虽然看似权限最安全，但在存储模型上不可行：

1. **碎片化灾难（Small File Problem）**
   - 用户 A 查部门 X，用户 B 查部门 Y，触发两次独立同步，分别向不同分区写入小增量。
   - 随着并发查询增加，Delta Lake 底层将堆积数千个极小的 Parquet 文件。
   - 元数据膨胀，查询性能断崖式下跌。`OPTIMIZE` 合并成本极高，且 sync-on-query 路径无法异步合并。

2. **数据僵尸（Data Zombies）**
   - 场景：用户 A 从部门 X 调动到部门 Y。
   - 后果：部门 X 的后续更新不再被同步（因为 A 无权访问）。Delta Lake 中永久保留部门 X 的旧版本数据。
   - 随着时间推移，湖中充满"无人能见"的僵尸数据，存储空间无限膨胀。

3. **同步空洞**
   - 长期无人查询的部门，数据永远停留在初始状态。这在方案 B 中是结构性缺陷。

### 选择：特权同步用户（系统级账号）

数据摄入（Sync）与数据消费（Query）严格解耦。

1. **单一事实来源（Single Source of Truth）**
   - Delta Lake 维护完整、最新的全量数据集。
   - 权限控制完全下沉到查询层，通过 `WHERE org_path LIKE '<prefix>%'` 强制执行。
   - 这是标准的**行级安全（Row-Level Security, RLS）**模型。

2. **低碎片化**
   - Sync 获取全量变更流，按分区写入连续的大文件块，避免零碎写入。

3. **风险缓解**
   - **特权账号注入**：通过环境变量注入，严禁打印到日志或写入配置文件明文。
   - **最小权限**：API 层限制该账号仅读（Read-Only），禁止任何写入操作。
   - **审计**：所有 Sync 操作记录独立审计日志。

**前置要求**：
必须提供一个拥有全组织读取权限的 API 同步账号。如果业务方无法提供或无法调整 API 逻辑，则此架构不可行，需评估其他方案（如直连业务库或接受性能劣化）。
