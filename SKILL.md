...
2. **日志初始化**：优先调用免配置的 `init_logging()`（读取框架 `settings.logging` 默认值，技能进程复用主进程行为）；如需自定义输出目标，再改用 `setup_logging(log_dir=...)` 手动传参（相对路径按当前工作目录解析）。`load_settings()` 建议显式传入消费方的 `skill_root`（即 `Path(__file__).resolve().parent.parent`），确保配置路径正确解析。
...
| 日志路径约束 | `log_dir` 相对路径相对于当前工作目录解析（输出目标二选一：空=终端，指定目录=文件按天旋转） | `references/structlog.md` |
...
- `log_dir` 相对路径相对于当前工作目录解析，如需输出到项目 logs 目录，使用 `../../logs`
